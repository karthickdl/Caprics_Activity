...
URL = "http://dlearners.in/template_and_games/Game_template_api-s/update_session_score.php"; // PRODUCTION
    ...
    IEnumerator EN_storeDataInDB()
    {
        ...
        form.AddField("score", DLearners.GameHandlerImmersiveGame.Instance.I_TotalPoints.ToString());
        form.AddField("rating", I_selectedStar.ToString());
        form.AddField("comments", IF_feedback.text);
        form.AddField("Si_no", DLearners.GameHandlerImmersiveGame.Instance.STR_responseSerial);

        Debug.Log("Store Values =" + DLearners.GameHandlerImmersiveGame.Instance.I_TotalPoints + "  ID" + DLearners.GameHandlerImmersiveGame.Instance.STR_responseSerial);
